...
export function detectCircle(points: Point[], width: number, height: number): CircleDetection | null {
  ...
  for (let i = 0; i < simplified.length; i += 2) {
    ...
    for (let radius = minRadius; radius <= maxRadius; radius += radiusStep) {
      for (const direction of [-1, 1]) {
        ...
        const votes = countRadialAgreement(simplified, snappedX, snappedY, radius, 0.16);
        if (votes > best.votes) best = { x: snappedX, y: snappedY, radius, votes };
      }
    }
  }
  ...
  const coverage = estimateCoverage(simplified, refined.x, refined.y, refined.radius);
  const error = radialError(simplified, refined.x, refined.y, refined.radius);
  const compactness = radiusConsistency(simplified, refined.x, refined.y, refined.radius);
  const errorScore = clamp(1 - error / Math.max(refined.radius * 0.22, 1), 0, 1);
  const finalScore = Math.round(clamp((errorScore * 0.65 + coverage * 0.2 + compactness * 0.15) * 100, 0, 100));
...
function simplifyPoints(points: Point[], minDistance: number) {
  ...
  for (let i = 1; i < points.length; i += 1) {
    ...
    if (Math.hypot(points[i].x - previous.x, points[i].y - previous.y) >= minDistance) result.push(points[i]);
  }
  return result;
}

function countRadialAgreement(points: Point[], centerX: number, centerY: number, radius: number, toleranceRatio: number) {
  const tolerance = Math.max(6, radius * toleranceRatio);
  let votes = 0;
  for (const point of points) {
    if (Math.abs(Math.hypot(point.x - centerX, point.y - centerY) - radius) <= tolerance) votes += 1;
  }
  return votes;
}

function refineCircle(points: Point[], centerX: number, centerY: number, radius: number) {
  ...
  for (let iteration = 0; iteration < 5; iteration += 1) {
    const near = points.filter(
      (point) => Math.abs(Math.hypot(point.x - currentX, point.y - currentY) - currentRadius) < currentRadius * 0.16,
    );
    ...
    currentRadius = near.reduce((sum, point) => sum + Math.hypot(point.x - currentX, point.y - currentY), 0) / near.length;
  }

  return { x: currentX, y: currentY, radius: currentRadius };
}

function radialError(points: Point[], centerX: number, centerY: number, radius: number) {
  if (points.length === 0) return Infinity;
  return points.reduce((sum, point) => sum + Math.abs(Math.hypot(point.x - centerX, point.y - centerY) - radius), 0) / points.length;
}

function radiusConsistency(points: Point[], centerX: number, centerY: number, radius: number) {
...
function estimateCoverage(points: Point[], centerX: number, centerY: number, radius: number) {
  const bins = new Set<number>();
  for (const point of points) {
    const distance = Math.hypot(point.x - centerX, point.y - centerY);
    if (Math.abs(distance - radius) > Math.max(8, radius * 0.14)) continue;
    const angle = ((Math.atan2(point.y - centerY, point.x - centerX) % TAU) + TAU) % TAU;
